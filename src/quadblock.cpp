...
std::vector<uint8_t> Quadblock::Serialize(size_t id, size_t offTextures, const std::vector<size_t>& vertexIndexes) const
{
	...
	else
	{
		...
		for (size_t i = 0; i < 10; i++)
			quadblock.triNormalVecDividend[i] = m_rawTriNormalVecDividend[i] ;
	}
...
void Quadblock::SetRawTriNormalVecDividend(const int16_t rawTriNormalVecDividend[10])
{
	for (size_t i = 0; i < 10; i++)
		m_rawTriNormalVecDividend[i] = rawTriNormalVecDividend[i];
}

void Quadblock::SetRawTriNormalVecBitshift(const uint8_t triNormalVecBitshift)
{
	m_triNormalVecBitshift = triNormalVecBitshift;
}

bool Quadblock::HasRawData() const
{ 
	return m_hasRawData;
}

void Quadblock::SetRawData(bool hasRawData) 
{
