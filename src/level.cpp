...
bool Level::LoadLEV(const std::filesystem::path& levFile)
{
	...
	if (texGroupsStart > header.offAnimTex) { texGroupsEnd = meshInfo.offQuadblocks; } // Is it guaranteed that quadblock is after textures ? Isn't the texture length stored somewhere ?
	uint32_t numTexGroups;
	if (texGroupsStart < texGroupsEnd) { numTexGroups = (texGroupsEnd - texGroupsStart) / sizeof(PSX::TextureGroup);}
	else { numTexGroups = 0; }
	m_rawTexGroups.clear();
	m_rawTexGroups.resize(numTexGroups);
	printf("start : 0x%x , end : 0x%x, size : 0x%x\n", texGroupsStart, texGroupsEnd, numTexGroups);
	...
	for (uint32_t i = 0; i < meshInfo.numQuadblocks; i++)
	{
		...
		quad.SetRawTriNormalVecDividend(quadblock.triNormalVecDividend);
		quad.SetRawTriNormalVecBitshift(quadblock.triNormalVecBitshift);
		quad.SetRawData(true);
...
bool Level::SaveLEV(const std::filesystem::path& path)
{
	...
	*/
	...
	// NEW: Check if we should use raw texture data (loaded but not modified)
	if (m_hasRawTextureData && !UpdateVRM())
	{
		// Use stored raw texture data - pass through unchanged
		texGroups = m_rawTexGroups;
		animData = m_rawAnimData;

		// Calculate animation data offset
		offAnimData = currOffset + (sizeof(PSX::TextureGroup) * texGroups.size());

		// Extract pointer map offsets from raw anim data
		if (!animData.empty())
		{
	...
	{
		// Generate textures from VRM (original code path)
