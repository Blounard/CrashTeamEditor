...
template class MaterialProperty<int, MaterialType::WEATHER_INTENSITY>;
template class MaterialProperty<int, MaterialType::WEATHER_VANISH_RATE>;
...
void MaterialProperty<T, M>::Apply(const std::string& material, const std::vector<size_t>& quadblockIndexes, std::vector<Quadblock>& quadblocks)
{
	...
	for (const size_t index : quadblockIndexes)
	{
		...
		if (!quadblock.GetHide())
		{
			...
			else if constexpr (M == MaterialType::WEATHER_INTENSITY) { quadblock.SetWeatherIntensity(preview); }
			else if constexpr (M == MaterialType::WEATHER_VANISH_RATE) { quadblock.SetWeatherVanishRate(preview); }
